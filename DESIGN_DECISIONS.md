...
## 13bis. Pairing de dispositivo — de auto-aprobación (descartada) a comando copiable

> Origen: el pairing se perdía en cada reinicio de PC porque el `IndexedDB` del navegador no siempre sobrevive entre sesiones (confirmado: 101 keypairs `webchat/ui` distintos generados por la misma PWA en `~/.openclaw/devices/paired.json`). El Gateway nunca perdió el estado — el problema era del lado del browser.

**Primer intento (descartado): botón de auto-aprobación desde el browser.** Abría una WebSocket temporal usando `GATEWAY_TOKEN` (device efímero) para auto-aprobar el device principal por loopback. Se descartó tras evidencia real de que el Gateway lo rechaza por diseño, en dos capas: (1) el browser siempre manda `Origin` header en el WS upgrade, y el Gateway solo auto-aprueba conexiones sin ese header (reservado para scripts CLI/Node.js); (2) mientras el device principal tiene una pairing request con scope de admin pendiente, el Gateway rechaza cualquier conexión nueva de ese mismo device, incluso con scopes reducidos. Ninguna de las dos es un bug — son protecciones intencionales, y el primer diseño intentaba sortear justo la que existe para que un token de admin expuesto en el bundle del cliente no sea trivialmente explotable. La restricción de seguridad que se había documentado acá (no exponer el PWA fuera de localhost sin rediseñar esto) resultó aplicar incluso dentro de localhost — el propio Gateway ya desconfiaba del intento antes de que la exposición externa fuera siquiera un tema.

**Solución actual: comando copiable, sin que el browser intente aprobar nada.** El botón en el header, visible cuando `connectionState === 'pairing_required'`, muestra `openclaw devices approve <requestId>` listo para copiar (con fallback a texto seleccionable si `clipboard.writeText()` falla) — la aprobación real sigue requiriendo una terminal con acceso legítimo a la máquina. Más lento que el botón mágico original, pero no reintroduce el problema de seguridad: el navegador nunca obtiene la capacidad de auto-aprobarse.

**Bug encontrado en el camino, ya corregido:** `NOT_PAIRED` no se clasificaba como caso terminal en `handleHandshakeError`, así que `scheduleReconnect` seguía disparando con backoff exponencial y pisaba el estado `pairing_required` apenas aparecía — el botón quedaba deshabilitado la mayor parte del tiempo por una carrera entre la reconexión automática y el estado de pairing, no por un problema de permisos de clipboard como parecía a primera vista. Corregido en tres puntos de `client.ts`: `handleHandshakeError` (NOT_PAIRED ahora es terminal, no reintenta), `onClose(1008)` (cancela cualquier timer de reconexión pendiente antes de setear `pairing_required`), `visibilityHandler` (no reconecta si el estado es `pairing_required`).

**Pendiente de confirmar:** el comportamiento estable del botón en el próximo reinicio real de PC (no simulado) — la corrección de la carrera de reconexión está verificada en el código pero no en el escenario completo de un reinicio genuino.
